...
- **Incremental**: each page stores a `source_hash`; unchanged files are skipped (no LLM call, no diff). Use `npm run wiki:ingest -- --force` to re-generate every page (e.g. to re-enrich with a better model). Override the model with `WIKI_INGEST_MODEL` (default `grok-4.7`).
