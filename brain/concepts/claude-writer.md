...
| Review gate | `autoblog-review.yml` | The `claude-reviewed` job applies `autoblog-approved-pending` on a pass, but only while the post's files are unchanged since the finalize commit, except for `[autoblog-cifix]` repairs. Any other edit waits for a human to add `autoblog-human-approved`, which approves the exact head it is applied to. A `[autoblog-cifix]` commit counts only if the post text with links stripped is unchanged. The Grok review and [[concepts/autofix-loop]] skip these PRs, and so does [[concepts/codex-heal]]. [[concepts/ci-heal]] still applies. |
