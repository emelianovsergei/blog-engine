...
Review. The gate counted the cancelled run as CI not green. The match
key is check type + workflow name + check name, so a success from a
different workflow does not hide a cancelled run. A FAILURE next to a
SUCCESS of the same key still fails.
